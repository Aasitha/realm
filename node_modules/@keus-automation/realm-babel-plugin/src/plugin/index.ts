...
import { types, NodePath, PluginObj, PluginPass, parse } from "@babel/core";

import { isPropertyImportedFromRealm, isImportedFromRealm, isRealmClassEmbedded } from "./import-checking";
...
function getRealmTypeForTypeArgument(
  ...
  if (typeParameters.isTSTypeParameterInstantiation()) {
    ...
    return getRealmTypeForTSType(objectTypePath, notPlainObject);
  }
}
...
function getRealmTypeForTSTypeReference(path: NodePath<types.TSTypeReference>, notPlainObject = false): RealmType | undefined {
  ...
  if (isRealmTypeAlias(path, "Bool")) {
    ...
    if (objectType?.type === 'object') {
      return { type: "list", objectType: objectType?.objectType, optional: objectType?.optional };
    } else {
    ...
    const objectType = getRealmTypeForTypeArgument(typeParameters);
    if (objectType?.type === 'object') {
      return { type: "set", objectType: objectType?.objectType, optional: objectType?.optional };
    } else {
    ...
    const objectType = getRealmTypeForTypeArgument(typeParameters);
    if (objectType?.type === 'object') {
      return { type: "dictionary", objectType: objectType?.objectType, optional: objectType?.optional };
    } else {
...
function getRealmTypeForTSType(path: NodePath<types.TSType>, notPlainObject = false): RealmType | undefined {
  if (path.isTSBooleanKeyword()) {
    ...
    return getRealmTypeForTSTypeReference(path, notPlainObject);
...
function visitRealmClass(path: NodePath<types.ClassDeclaration>) {
  ...
  if (isRealmClassEmbedded(schemaStatics)) {
    path.insertAfter(types.exportNamedDeclaration(types.variableDeclaration("const", [
      types.variableDeclarator(types.identifier(className), schema)
    ])));
    path.remove();
  } else {
    ...
    visitor: {
      ...
      VariableDeclaration(path) {
        ...
        if (path.node && path.node.declarations) {
          console.log('Variable Declaration', path.node.declarations[0].id);
        }
        console.log("===============================")
      },
      ExportDeclaration(path) {
        console.log("+++++++++++++++++++++++++++++++")
        if (path.node && path.node) {
          console.log('Export Declaration', path.node);
        }
        console.log("===============================")
      }
    },
