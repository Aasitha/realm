...
export function isImportedFromRealm(path: NodePath<types.Node>): boolean {
  if (path.isMemberExpression()) {
    ...
    // @ts-ignore
    path.isDecorator() && types.isCallExpression(path.get("expression")) && types.isMemberExpression(path.get("expression").get("callee"))
  ) {
...
export function isRealmClassEmbedded(schemaStatics: Array<any>): boolean {
