...
export const BaseSchemaName = "IKeusGroupSchema";
...
//     }
// }
export class IKeusGroupSchema extends Realm.Object<IKeusGroupSchema>{
...
export class Methods {
    ...
    static async getGroupId(realm: Realm, data: any) {
        return data.groupRoom + '-' + data.groupId;
    }
    static async addGroup(realm: Realm, doc: any) {
        // console.log(doc._id)
    ...
    static async getGroupById(realm: Realm, data: any) {
        return realm.objects(BaseSchemaName).filtered('groupRoom==$0 AND groupId==$1', data.roomId, data.groupId)[0]
    }
    static async getGroupsByRooms(realm: Realm, roomIds: Array<String>) {
    ...
    static async deleteGroupById(realm: Realm, data: any) {
        realm.write(() => {
            realm.delete(realm.objects(BaseSchemaName).filtered('groupRoom == $0 AND groupId== $1', data.roomId, data.groupId))
        })
    }
    static async updateGroupDeviceList(realm: Realm, data: any) {
        realm.write(() => {
    ...
    static async configureGroupProperties(realm: Realm, data: any) {
        realm.write(() => {
            let group = realm.objects(BaseSchemaName).filtered('groupRoom == $0 AND groupId== $1', data.roomId, data.groupId)[0];
    ...
    static async changeGroupName(realm: Realm, data: any) {
        realm.write(() => {
            let group = realm.objects(BaseSchemaName).filtered('groupRoom == $0 AND groupId== $1', data.roomId, data.groupId)[0];
    ...
    static async updateGroupState(realm: Realm, data: any) {
        realm.write(() => {
            let group = realm.objects(BaseSchemaName).filtered('groupRoom == $0 AND groupId== $1', data.roomId, data.groupId)[0];
            group.groupState = JSON.stringify(data.groupState);
        })
    }
    static async updateGroupStateAndSource(realm: Realm, data: any) {
        realm.write(() => {
            let group = realm.objects(BaseSchemaName).filtered('groupRoom == $0 AND groupId== $1', data.roomId, data.groupId)[0];
            group.groupState = JSON.stringify(data.groupState);
    ...
    static async cleanDriverGroups(realm: Realm, driverList: Array<string>) {
        ...
        realm.write(() => {
            groups.forEach((group: any) => {
                group.deviceList = group.deviceList.filter((device: any) => {
                    return !driverList.includes(device);
                })
            })
        })
    }
    static async cleanEmbeddedApplianceGroups(realm: Realm, deviceId: string) {
    ...
    static async updateGroupSyncState(realm: Realm, data: any) {
        realm.write(() => {
            let group = realm.objects(BaseSchemaName).filtered('groupRoom == $0 AND groupId== $1', data.roomId, data.groupId)[0];
    ...
    static async replaceGroupDeviceList(realm: Realm) {
